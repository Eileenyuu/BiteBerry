...
async def recommend_recipe(user_id: int, max_budget: float = None, max_cooking_time: int = None, db: Session = Depends(get_db)):
    """
        ...
        if recipes:  # Check if recipes exist
            for recipe in recipes:
                ...
                if (recipe.budget <= final_budget and 
                    recipe.cooking_time <= final_cooking_time):
